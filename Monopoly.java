 ...
 * int de2 = lancerDe();
 * int totalDe = de1 + de2;
 * System.out.println(j.getNom() + " lance les dés et fait " + de1 + " + " + de2
 * + " = " + totalDe);
 ...
 * System.out.println(j.getNom() + " tombe sur la case N° " + positionCourante +
 * " : " + c.getNom());
 * j.setPosition(positionCourante);
 * if (c instanceof Terrain) {
 * Propriete p = (Propriete) c;
 * if (p.getProprietaire() == null) {
 * System.out.print(j.getNom() + " veut-il acheter la " + p.getNom() + " (" +
 * p.getPrix()
 * + " euros) (o-n)? ");
 ...
 * System.out.println(
 * j.getNom() + " a payé " + p.getLoyer() + " à " +
 * p.getProprietaire().getNom());
 * }
 ...
 * " passe par la case départ et reçoit 200 euros");
 * }
 * if (j.getArgent() < 0) {
